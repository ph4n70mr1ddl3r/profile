...
use profile_shared::crypto::{generate_private_key, derive_public_key, sign_message, verify_signature};
use serde_json::json;
...
fn test_wrong_public_key_rejected() {
    ...
    let signature = sign_message(&private_key1, b"auth")
        .expect("Should create signature");
...
fn test_hex_encoding_roundtrip() {
    ...
    assert!(result.is_ok(), "Hex roundtrip should preserve signature validity");
}
...
fn test_case_insensitive_hex_decoding() {
    ...
    assert_eq!(decoded_lower, decoded_upper, "Hex decoding should be case-insensitive");
