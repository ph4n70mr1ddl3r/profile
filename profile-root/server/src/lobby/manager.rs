...
pub async fn add_user(
    ...
    if is_reconnection {
        broadcast_user_left(lobby, &key)
            .await
            .map_err(|_| LobbyError::BroadcastFailed)?;
    }
    // Always broadcast "joined" for new/reconnected user
    broadcast_user_joined(lobby, &key)
        .await
        .map_err(|_| LobbyError::BroadcastFailed)?;
...
mod tests {
    ...
    async fn test_add_user_invalid_key() {
        ...
        let result = add_user(
            &lobby,
            "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcde".to_string(),
            connection.clone(),
        )
        .await;
        assert_eq!(result, Err(LobbyError::InvalidPublicKey));
        ...
        let result = add_user(
            &lobby,
            "1234567890abcdef1234567890abcdef1234567890abcdef1234567890zzzzyy".to_string(),
            connection.clone(),
        )
        .await;
        assert_eq!(result, Err(LobbyError::InvalidPublicKey));
    ...
    async fn test_add_user_key_length_boundary() {
        ...
        let key_65 =
            "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1".to_string();
        ...
        let key_66 =
            "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef12".to_string();
        assert_eq!(key_66.len(), 66);
        let result = add_user(&lobby, key_66, connection.clone()).await;
        assert_eq!(
            result,
            Err(LobbyError::InvalidPublicKey),
            "66-char key should be rejected"
        );
    ...
    async fn test_remove_user_deletes_entry() {
        ...
        // Add user first
        add_user(&lobby, connection_key.clone(), connection)
            .await
            .unwrap();
    ...
    async fn test_broadcast_sends_delta_format() {
        ...
        crate::lobby::add_user(&lobby, mock_connection.public_key.clone(), mock_connection)
            .await
            .unwrap();

        // Add the actual user (this should trigger broadcast)
        crate::lobby::add_user(&lobby, connection_key.clone(), connection)
            .await
            .unwrap();
    ...
    async fn test_message_routing_uses_sender() {
        ...
        let connection2 = ActiveConnection {
            public_key: "ccdd1234567890abcdef1234567890abcdef1234567890abcdef1234567890cd"
                .to_string(),
            sender: sender2,
            connection_id: 2,
        };
        ...
        let key1 = connection1.public_key.clone();
        let key2 = connection2.public_key.clone();
        crate::lobby::add_user(&lobby, key1.clone(), connection1)
            .await
            .unwrap();
        crate::lobby::add_user(&lobby, key2.clone(), connection2)
            .await
            .unwrap();
        ...
        if let (Some(user1_conn), Some(user2_conn)) = (user1_result, user2_result) {
            ...
            match (received1, received2) {
                (
                    profile_shared::Message::Text { message: text1, .. },
                    profile_shared::Message::Text { message: text2, .. },
                ) => {
    ...
    async fn test_concurrent_add_remove_safe() {
        ...
        for i in 0..50 {
            ...
            // Add user
            let add_result =
                crate::lobby::add_user(&lobby, connection_key.clone(), connection).await;
        ...
        assert_eq!(
            users.len(),
            0,
            "Lobby should be empty after rapid operations"
        );
        ...
        assert_eq!(
            final_users.len(),
            10,
            "Expected 10 users remaining, got {}. Users: {:?}",
            final_users.len(),
            final_users
        );
    ...
    async fn test_ghost_user_prevention() {
        ...
        crate::lobby::remove_user(&lobby, &connection_key)
            .await
            .unwrap();
        ...
        let lookup_result = crate::lobby::get_user(&lobby, &connection_key)
            .await
            .unwrap();
        ...
        crate::lobby::add_user(&lobby, new_connection_key.clone(), new_connection)
            .await
            .unwrap();
    ...
    async fn test_broadcast_latency_within_100ms() {
        ...
        let connection = create_test_connection(
            "latency_test_user_1234567890abcdef1234567890abcdef1234567890ab",
        );
        let connection_key = connection.public_key.clone();
    ...
    async fn test_leave_broadcast_latency_within_100ms() {
        ...
        let _ =
            tokio::time::timeout(std::time::Duration::from_millis(10), test_receiver.recv()).await;

        // Create a second user who will disconnect
        let connection =
            create_test_connection("latency_test_user_leave_1234567890abcdef1234567890ab");
        let connection_key = connection.public_key.clone();
