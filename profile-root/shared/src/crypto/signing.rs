...
fn convert_private_key_to_signing_key(private_key: &Zeroizing<Vec<u8>>) -> Result<SigningKey, CryptoError> {
    let private_key_bytes: [u8; 32] = private_key.as_slice().try_into()
...
mod tests {
    ...
    fn test_sign_message_deterministic_10k() {
        ...
        let first_signature = sign_message(&private_key, message).expect("First signature should succeed");
