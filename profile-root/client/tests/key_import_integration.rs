...
async fn integration_test_import_hex_and_derive() {
    ...
    let original_key = profile_shared::generate_private_key()
        .expect("Key generation should succeed");
    let hex_input = hex::encode(&*original_key);
    
    // Simulate handler validation: decode hex
    let decoded_key = Zeroizing::new(
        hex::decode(&hex_input).expect("Hex decode should succeed for valid input")
    );
    ...
    let public_key = profile_shared::derive_public_key(&decoded_key)
        .expect("Derivation should succeed");
    
    // Verify properties
    assert_eq!(public_key.len(), 32, "Public key must be 32 bytes");
    assert_ne!(&decoded_key[..], &public_key[..], "Keys should be different");
...
async fn integration_test_import_matches_generation() {
    ...
    let generated_key = profile_shared::generate_private_key()
        .expect("Key generation should succeed");
    let generated_public = profile_shared::derive_public_key(&generated_key)
        .expect("Derivation should succeed");
    
    // Import the same key (Story 1.2 path: hex encode -> decode -> derive)
    let hex_input = hex::encode(&*generated_key);
    let imported_key = Zeroizing::new(
        hex::decode(&hex_input).expect("Hex decode should succeed")
    );
    let imported_public = profile_shared::derive_public_key(&imported_key)
        .expect("Derivation should succeed");
...
async fn integration_test_import_performance() {
    ...
    for _ in 0..10 {
        ...
        let _public_key = profile_shared::derive_public_key(&decoded_key)
            .expect("Derivation should succeed");
...
async fn integration_test_concurrent_import_safe() {
    ...
    for _ in 0..10 {
        ...
        let task = tokio::spawn(async move {
            let decoded_key = Zeroizing::new(
                hex::decode(&hex_clone).expect("Hex decode should succeed")
            );
            profile_shared::derive_public_key(&decoded_key)
                .expect("Derivation should succeed")
        });
        tasks.push(task);
    }
...
async fn integration_test_case_insensitive_import() {
    ...
    let decoded_lower = Zeroizing::new(
        hex::decode(&lowercase_hex).expect("Lowercase should decode")
    );
    let decoded_upper = Zeroizing::new(
        hex::decode(&uppercase_hex).expect("Uppercase should decode")
    );
    ...
    let public_lower = profile_shared::derive_public_key(&decoded_lower)
        .expect("Derivation should succeed");
    let public_upper = profile_shared::derive_public_key(&decoded_upper)
        .expect("Derivation should succeed");
...
async fn integration_test_import_with_async_state() {
    ...
    let public_key = {
        let guard = key_state.lock().await;
        profile_shared::derive_public_key(&*guard)
            .expect("Derivation should succeed")
    };
