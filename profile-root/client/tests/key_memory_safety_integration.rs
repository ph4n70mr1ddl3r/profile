...
fn test_memory_not_persisted_to_disk() {
    ...
    // Count files before
    let before_files: Result<Vec<_>, _> = fs::read_dir(&temp_dir)
    ...
    // Count files after
    let after_files: Result<Vec<_>, _> = fs::read_dir(&temp_dir)
        .map(|entries| entries.collect());
    let after_count = after_files.map(|v| v.len()).unwrap_or(0);
    
    assert_eq!(before_count, after_count, "No files should be created during key operations");
}
...
fn test_private_key_not_in_display_or_debug() {
    ...
    assert!(debug_output.contains("Zeroizing"), 
...
fn test_zeroize_on_error_path() {
    ...
    assert!(result.is_err(), "Derivation should fail with wrong-length key");
