...
async fn test_client_preserves_draft_on_disconnect() {
    ...
    let mut client = WebSocketClient::new(key_state);
    let _ = client
        .handle_disconnection("network_error".to_string())
        .await;
...
async fn test_client_displays_error_on_server_close() {
    ...
    let result = client
        .handle_disconnection("server_shutdown".to_string())
        .await;
    assert!(result.is_err());
...
async fn test_keys_remain_in_memory_after_disconnect() {
    ...
    assert!(
        ...
        "Private key should remain in memory"
    );
    assert!(
        state.public_key().is_some(),
        "Public key should remain in memory"
    );
}
