...
fn main() {
    ...
    let mut lobby = lobby;
    let test_users = vec![
        LobbyUser::new("aaaa1111222233334444555566667777aaa111222233".to_string(), true),
        LobbyUser::new("bbbb1111222233334444555566667777bbb111222233".to_string(), true),
        LobbyUser::new("cccc1111222233334444555566667777ccc111222233".to_string(), true),
    ];
    ...
    for user in &test_users {
        ...
        let end = if pk.len() >= 8 { &pk[pk.len()-8..] } else { "" };
    ...
    for user in lobby.users() {
        ...
        let status = if is_selected { "👆 SELECTED" } else { "  normal" };
        let pk = &user.public_key;
        let start = if pk.len() >= 8 { &pk[..8] } else { pk };
        let end = if pk.len() >= 8 { &pk[pk.len()-8..] } else { "" };
    ...
    lobby.add_user(LobbyUser::new("user1_aaaaa1111222233334444555566aa".to_string(), true));
    lobby.add_user(LobbyUser::new("user2_bbbbb1111222233334444555566bb".to_string(), true));
    lobby.add_user(LobbyUser::new("user3_ccccc1111222233334444555566cc".to_string(), true));
    ...
    println!("User count (lobby_user_count): {}", lobby.len());
    if let Some(key) = selected_key {
        println!("Selected user (lobby_selected_user): {}...{}", &key[..8], &key[56..]);
    } else {
        println!("Selected user (lobby_selected_user): \"\"");
    }
    println!();
    ...
    println!("--------------------------------------");
    let mut lobby = LobbyState::new();
    lobby.add_user(LobbyUser::new("user1_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".to_string(), true));
    lobby.add_user(LobbyUser::new("user2_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb".to_string(), true));
    lobby.add_user(LobbyUser::new("user3_ccccccccccccccccccccccccccccccc".to_string(), true));
