...
impl ClientAuthMessage {
    ...
    pub fn new(public_key: Vec<u8>, private_key: Zeroizing<Vec<u8>>) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
...
mod tests {
    ...
    async fn test_client_auth_message_creation() {
        ...
        assert!(result.is_ok(), "Client auth message creation should work: {:?}", result.err());
    ...
    async fn test_client_auth_message_json_serialization() {
        ...
        assert!(result.is_ok(), "Auth message JSON serialization should work: {:?}", result.err());
    }
