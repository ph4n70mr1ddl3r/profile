...
mod tests {
    ...
    async fn test_concurrent_generation_requests_are_safe() {
        ...
        let task1 = tokio::spawn(async move { handle_generate_new_key(&key_state).await });

        let task2 = tokio::spawn(async move { handle_generate_new_key(&key_state_clone).await });
        ...
        assert_ne!(
            result1.unwrap(),
            result2.unwrap(),
            "Concurrent generations should produce different keys"
        );
    }
    ...
    async fn test_generation_is_idempotent_when_called_sequentially() {
        ...
        assert_ne!(
            key1, key2,
            "Sequential generations should produce different keys"
        );
        assert_ne!(
            key2, key3,
            "Sequential generations should produce different keys"
        ...
        assert!(state.is_key_set());
        assert_eq!(
            hex::encode(state.public_key().unwrap()),
            key3,
            "State should contain last generated key"
        );
    }
}
